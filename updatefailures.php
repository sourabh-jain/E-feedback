...
$current_activity = "At Update Failures Page";
$user = $_SESSION['use'];
	...
	?>
				...
				<div class="alert alert-info alert-dismissable">
                                        ...
                                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                                         Enter rollno of students who failed(Seperated by new line)
                                    </div>
				
				<form name='myfrm' action='' method='post'>
				
				<textarea name="failuresinfo" rows="10" class="form-control"></textarea>
				...
				{
					/* Get all rollnos */
					$lines=explode(PHP_EOL,$_POST['failuresinfo']);
					foreach($lines as $roll)
					{
						/* Check if its not already a back year student */
						$result=mysqli_query($con,"SELECT * from user where rollno='$roll'");
		...
		
						{
							...
							$re=mysqli_query($con,"SELECT * from half_year_setup");
							$rw=mysqli_fetch_array($re);
						
								mysqli_query($con,"UPDATE user set semester=semester - 2, backyear='YES' where rollno='$roll'");
								if($rw['semtype']=='ODD')  /* If it is odd sem then allow user to fill feedback else not */
									mysqli_query($con,"UPDATE user set allowed='YES' where rollno='$roll'");
								else
									mysqli_query($con,"UPDATE user set allowed='NO' where rollno='$roll'");
								
							echo '
							<div class="alert alert-success alert-dismissable">
                                        <i class="fa fa-check"></i>
                                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                                         Updated ' . $roll . '(' . $row['name'] . ')
                                    </div>';
						...
						else
						{
							echo '<div class="alert alert-danger alert-dismissable">
                                        <i class="fa fa-ban"></i>
                                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                                        The user  '. $roll.'(' . $row['name'] . ')  was already updated! 
                                    </div>';
						}
